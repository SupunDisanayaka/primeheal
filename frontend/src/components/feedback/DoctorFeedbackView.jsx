...
const DoctorFeedbackView = ({ doctorId: propDoctorId = null }) => {
  ...
  const resolvedDoctorId =
    propDoctorId ||
    appContext?.userData?.doctorID ||
    appContext?.userData?.userID ||
  ...
  const fetchDoctorFeedback = useCallback(async () => {
    ...
    try {
      ...
      const token = sessionStorage.getItem('token') || sessionStorage.getItem('doctorToken');
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center space-x-3">
                      <div className="w-10 h-10 rounded-full bg-primary/10 text-primary font-bold flex items-center justify-center text-base shrink-0">
                        {initial}
                      </div>
                      <div>
